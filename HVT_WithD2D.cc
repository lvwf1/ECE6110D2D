...
int main(int argc, char *argv[]) {
    ...
    // Create D2D channels between two user mobiles
    double d2dDelay = 1.0;
    uint64_t d2dDataRate = 3000000000;

    pointHelper.SetChannelAttribute("Delay", TimeValue(MilliSeconds(d2dDelay)));
    pointHelper.SetDeviceAttribute("DataRate", DataRateValue(DataRate(d2dDataRate)));
    NetDeviceContainer dev5 = pointHelper.Install(userNodes.Get(0), userNodes.Get(1));
    ...
    Ipv4InterfaceContainer ip5 = ipv4AddressHelper.Assign(dev5);

    Ipv4GlobalRoutingHelper::PopulateRoutingTables();

    // Create UDP Server application on base station node
    UdpServerHelper server1(port);
    ApplicationContainer serverApp = server1.Install(baseNode.Get(0));
    ...
    UdpClientHelper client2(ip1.GetAddress(0), port);
    ...
    ApplicationContainer app2 = client2.Install(userNodes.Get(1));
    ...
    ApplicationContainer app3 = client3.Install(userNodes.Get(2));
    app3.Start(Seconds(2.0));
    app3.Stop(Seconds(simTime));

    UdpClientHelper client4(ip2.GetAddress(0), port);
    ...
    ApplicationContainer app4 = client3.Install(userNodes.Get(3));
